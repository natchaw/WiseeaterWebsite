 ...
 * @since 2.0.2 Ajout de l'attribut 'loading' aux images
 *              Ajout des attributs d'édition en ligne
 */
...
class Team_Members_Widget extends Widget_Base {
	...
	public function __construct( $data = array(), $args = null ) {
		...
		wp_register_style( 'eac-team-members', EAC_Plugin::instance()->get_register_style_url( 'team-members' ), array( 'eac' ), EAC_ADDONS_VERSION );
	}
	...
	private function get_social_medias( $repeater_item ) {
		...
		foreach ( $social_medias as $site => $icon ) {
			...
			if ( 'email' === $site ) {
				$value .= '<a href="' . esc_url( 'mailto:' . antispambot( sanitize_email( $repeater_item[ 'tm_member_social_' . $site ] ) ) ) . '" rel="nofollow">';
			} else {
				$value .= '<a href="' . esc_url( $repeater_item[ 'tm_member_social_' . $site ] ) . '" target="_blank" rel="nofollow noopener noreferrer">';
			}
			$value .= '<span class="dynamic-tags_social-icon ' . esc_attr( $site ) . '" title="' . esc_attr( ucfirst( $site ) ) . '">';
